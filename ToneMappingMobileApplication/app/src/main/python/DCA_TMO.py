...
def DCA_TMO(hdrImg):
    ...
    labels_DoG = labels + 3.0 * convolve(hdrPQnor,DoGfilter, mode='nearest')
    ...
    tempHdrImg[:,:,0] = pow(tempHdrImg[:,:,0] / hdrLum,s) * labels_DoG
    tempHdrImg[:,:,1] = pow(tempHdrImg[:,:,1] / hdrLum,s) * labels_DoG
    tempHdrImg[:,:,2] = pow(tempHdrImg[:,:,2] / hdrLum,s) * labels_DoG
    ldrImg_DoG = tempHdrImg 
